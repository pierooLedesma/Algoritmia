...
// Módulo en que el usuario ingresa el presupuesto y el nivel de seguridad mínimo
// que el usuario desea para los recursos de seguridad.
...
// y que esté en el nivel de seguridad mínimo requerido. Además, valida las dependencias
// entre los recursos si existieran.
void procesarCombinaciones(const int presupuesto, const int seguridad_minima,
    ...
    // Mostrar el rango de costo y nivel de seguridad permitido.
    cout << "Combinaciones validas (Costo entre " << limite_inferior;
    cout << " y " << limite_superior << " miles y Nivel de Seguridad >= ";
    cout << seguridad_minima << "):" << endl;
    ...
    for (int iteracion = 0; iteracion < combinaciones; iteracion++) {
        ...
        int seguridad_total = 0; // Variable que acumula el nivel de seguridad de los recursos seleccionados.
        ...
        for (int i=0; i<MAX_RECURSOS; i++) {
            if (combinaciones[i]) { // Si el recurso está seleccionado
                ...
                costo_total += recursos[i].costo; // Sumar el costo del recurso seleccionado.
                seguridad_total += recursos[i].seguridad; // Sumar el nivel de seguridad del recurso seleccionado.
            }
        }

        // Si la combinación es válida, el costo está dentro del rango permitido y
        // el nivel de seguridad es mayor o igual que el nivel deseado por el usuario.
        if (combinacion_valida and costo_total <= limite_superior and
            costo_total >= limite_inferior and  seguridad_total >= seguridad_minima) {
            // Imprimir la combinación con su costo y nivel de seguridad total.
            imprimir_combinacion(iteracion, combinaciones, MAX_RECURSOS, costo_total, seguridad_total);
...
void imprimir_combinacion(const int iteracion, const int *combinaciones, const int cant_recursos,
    ...
    cout << "| Costo: " << costo_total << " miles | Nivel de Seguridad: " << seguridad_total << endl;
}
